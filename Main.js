...
/**
 * @description 處理所有「新增」相關的對話流程 (修正手動輸入單位邏輯)
 */
function handleAddFlow(event, token) {
  const userProperties = PropertiesService.getUserProperties();
  const state = userProperties.getProperty('state');
  const eventType = event.type;
  const userId = event.source.userId;
...
      if (eventType !== 'postback' || data.split('=')[0] !== 'add_category') return;
      tempData.分類 = value;
      nextState = 'add_awaiting_name';
      replyMessages.push({ type: 'text', text: getConfigMessage('PROMPT_ADD_NAME', { category: tempData.分類 }) });
      break;
...
      replyMessages.push({ type: 'text', text: getConfigMessage('PROMPT_ADD_MODEL', { name: tempData.品名 }), quickReply: { items: modelButtons }});
      break;
...
      replyMessages.push({ type: 'text', text: getConfigMessage('PROMPT_ADD_SPEC', { model: tempData.型號 }), quickReply: { items: specButtons }});
      break;
      
...
      replyMessages.push({ type: 'text', text: getConfigMessage('PROMPT_ADD_UNIT', { spec: tempData.規格 }), quickReply: { items: unitButtons } });
      break;
      
...
        replyMessages.push({ type: 'text', text: getConfigMessage('PROMPT_ADD_QUANTITY', { unit: tempData.單位 }) });
      }
      break;
...
      const confirmText = getConfigMessage('PROMPT_ADD_CONFIRM', { category: tempData.分類, name: tempData.品名, model: tempData.型號, spec: tempData.規格, unit: tempData.單位, quantity: tempData.數量 });
...
      if (value === '確認新增') {
        if (doesMaterialExist(tempData)) {
          const errorMessage = getConfigMessage('ERROR_DUPLICATE_ITEM', {
            name: tempData.品名,
            model: tempData.型號,
            spec: tempData.規格
          });
...
        } else {
          tempData.序號 = generateNewSerial(tempData.分類);
          tempData.類型 = '新增';
          const userName = JSON.parse(getUserProfile(userId, token)).displayName;
          addTransactionRecord(tempData, userName);
          replyMessages.push({ type: 'text', text: getConfigMessage('MSG_ADD_SUCCESS', { id: `${tempData.分類}${tempData.序號}` }) });
        }
      } else {
        replyMessages.push({ type: 'text', text: getConfigMessage('MSG_CANCEL_CONFIRM') });
      }
